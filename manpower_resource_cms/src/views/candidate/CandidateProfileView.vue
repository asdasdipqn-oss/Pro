...
const handleSave = async () => {
  ...
  try {
    ...
    // 保存成功后重新加载数据
    console.log('[CandidateProfile] 保存成功，重新加载数据...')
    await loadProfile()
  } catch (error) {
...
const loadProfile = async () => {
  try {
    console.log('[CandidateProfile] 开始加载个人信息...')
    const response = await request.get('/candidate/profile')
    console.log('[CandidateProfile] 加载个人信息响应:', response)

    const data = response.data.data
    if (data) {
      console.log('[CandidateProfile] 原始数据:', data)
      ...
      console.log('[CandidateProfile] 表单数据加载完成:', form)
    } else {
      console.log('[CandidateProfile] 没有找到个人信息数据')
    }
  } catch (error) {
...
const loadHistory = async () => {
  try {
    console.log('[CandidateProfile] 开始加载历史记录...')
    const response = await request.get('/candidate/profile/history')
    console.log('[CandidateProfile] 加载历史记录响应:', response)

    if (response.code === 200 && response.data) {
      historyList.value = response.data || []
      console.log('[CandidateProfile] 历史记录数据:', historyList.value)
    }
  } catch (error) {
